  ...
  <section id="mypage-container" v-if="$store.state.searchUserInfo !== null">
    ...
    <article id="profile-container">
      ...
      <div id="profile-card">
        ...
        <div id="info-card">
          <h3>이야기 0개</h3>
          <h3 @click="showFollowerList" style="cursor: pointer;">팔로우 {{ this.$store.state.searchUserFollowInfo.userFollow.length }}</h3>
          <h3 @click="showFollowingList" style="cursor: pointer;">팔로잉 {{ this.$store.state.searchUserFollowInfo.userFollowing.length }}</h3>
        </div>
      </div>
    </article>
    <article id="tab">
